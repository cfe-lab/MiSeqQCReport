...
sub uploadSummaryValues($$) {
    ...
    my $query = "SELECT read1, index1, index2, read2 FROM MiSeqQC_RunParameters WHERE runID LIKE ?";
    my $sth = $db->prepare($query); $sth->execute($RunID);
    ...
    # % Aligned for read 1
    $query = q{
        SELECT  AVG(Value) aligned
        FROM    MiSeqQC_TileMetrics
        WHERE   runID LIKE ?
        AND     metricCode LIKE ?
    };
    $sth = $db->prepare($query);
    $sth->execute($RunID, $metricCode);
    ...
    $query = q{
        SELECT  AVG(INTENSITY_A),
                AVG(INTENSITY_C),
                AVG(INTENSITY_G),
                AVG(INTENSITY_T)
        FROM    MiSeqQC_ExtractionMetrics
        WHERE   RunID LIKE ?
        AND     cycle IN 
                (
                SELECT  MIN(cycle)
    ...
    # Read 1
    $query = q{
        SELECT  SUM(numClusters)
        FROM    MiSeqQC_QualityMetrics
        WHERE   RunID LIKE ?
        AND     cycle > 0
        AND     cycle <= ?
        AND Q_Bin < 30
    ...
    $query = q{
        SELECT  ROUND(AVG(value)*100,3)
        FROM    MiSeqQC_TileMetrics
        WHERE   RunID LIKE ?
        AND     metricCode IN 
                (
                SELECT  MIN(metricCode)
                FROM    MiSeqQC_TileMetrics
                WHERE   RunID LIKE ?
                AND     metricCode LIKE '20%'
                AND     MOD(metricCode,2) = 0
                )
    };
    $sth = $db->prepare($query);
    $sth->execute($RunID, $RunID);
