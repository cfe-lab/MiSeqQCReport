...
if (!defined($ARGV[0])) { die "Syntax: $0 <runID>"; } else { $RunID = $ARGV[0]; $RunID =~ s/[%]//g; }

#3345#

my $query = "SELECT * FROM specimen.MiSeqQC_interopsummary WHERE runID LIKE '$RunID'";
...
$query = "SELECT read1, index1, index2, read2 FROM specimen.MiSeqQC_RunParameters WHERE runID LIKE '$RunID'";
...
if (my @row = $sth->fetchrow_array()) { ($read1, $index1, $index2, $read2) = @row }
	...
	print 	"\nERROR: RunID doesn't appear to exist!\n" .
			"Displaying RunIDs in emartin.runparameters for which there is no corresponding row in emartin.interopsummary...\n\n";
...
# % Aligned for read 1
$query = 	"SELECT AVG(Value) aligned FROM specimen.MiSeqQC_TileMetrics " .
...
$query = 	"SELECT AVG(INTENSITY_A), AVG(INTENSITY_C), AVG(INTENSITY_G), AVG(INTENSITY_T) FROM specimen.MiSeqQC_ExtractionMetrics " .
			"WHERE RunID LIKE '$RunID' AND cycle IN " .
			"(SELECT MIN(cycle) FROM specimen.MiSeqQC_ExtractionMetrics WHERE RunID LIKE '$RunID' AND $read1)";
$sth = $db->prepare($query); $sth->execute();
...
$query =	"SELECT AVG(INTENSITY_A), AVG(INTENSITY_C), AVG(INTENSITY_G), AVG(INTENSITY_T) FROM specimen.MiSeqQC_ExtractionMetrics WHERE RunID LIKE '$RunID' AND cycle IN " .
			"19 + (SELECT MIN(cycle) FROM specimen.MiSeqQC_ExtractionMetrics WHERE RunID LIKE '$RunID' AND $read1)";
$sth = $db->prepare($query); $sth->execute();
...
$query =	"SELECT AVG(INTENSITY_A), AVG(INTENSITY_C), AVG(INTENSITY_G), AVG(INTENSITY_T) FROM specimen.MiSeqQC_ExtractionMetrics WHERE RunID LIKE '$RunID' AND cycle IN " .
		"(SELECT MIN(cycle) FROM specimen.MiSeqQC_ExtractionMetrics WHERE RunID LIKE '$RunID' AND $read2)";
$sth = $db->prepare($query); $sth->execute();
...
$query =	"SELECT AVG(INTENSITY_A), AVG(INTENSITY_C), AVG(INTENSITY_G), AVG(INTENSITY_T) FROM specimen.MiSeqQC_ExtractionMetrics WHERE RunID LIKE '$RunID' AND cycle IN " .
		"19 + (SELECT MIN(cycle) FROM specimen.MiSeqQC_ExtractionMetrics WHERE RunID LIKE '$RunID' AND $read2)";
$sth = $db->prepare($query); $sth->execute();
...
# Read 1
$query = "SELECT SUM(numClusters) FROM specimen.MiSeqQC_QualityMetrics WHERE RunID LIKE '$RunID' AND $read1 AND Q_Bin < 30"; $sth = $db->prepare($query); $sth->execute();
...
# Read 2
$query = "SELECT SUM(numClusters) FROM specimen.MiSeqQC_QualityMetrics WHERE RunID LIKE '$RunID' AND $read2 AND Q_Bin < 30"; $sth = $db->prepare($query); $sth->execute();
...
#We don't need this data any more, so lets get rid of it.
$query = "delete from specimen.MiSeqQC_QualityMetrics where cycle not in (50, 260,500)";
$sth = $db->prepare($query); $sth->execute();
$db->commit();
...
$query = "SELECT AVG(value) FROM specimen.MiSeqQC_TileMetrics WHERE runID LIKE '$RunID' AND metricCode LIKE '100'";
$sth = $db->prepare($query); $sth->execute();
...
$query = "SELECT SUM(value) FROM specimen.MiSeqQC_TileMetrics WHERE runID LIKE '$RunID' AND metricCode LIKE '102'";
$sth = $db->prepare($query); $sth->execute();
...
$query = "SELECT SUM(value) FROM specimen.MiSeqQC_TileMetrics WHERE runID LIKE '$RunID' AND metricCode LIKE '103'";
$sth = $db->prepare($query); $sth->execute();
...
$query =	"SELECT ROUND(AVG(value)*100,3) FROM specimen.MiSeqQC_TileMetrics WHERE RunID LIKE '$RunID' AND metricCode IN " .
		"(SELECT MIN(metricCode) FROM specimen.MiSeqQC_TileMetrics WHERE RunID LIKE '$RunID' AND metricCode LIKE '20%' AND MOD(metricCode,2) = 0)";
$sth = $db->prepare($query); $sth->execute();
...
$query =	"SELECT ROUND(AVG(value)*100,3) FROM specimen.MiSeqQC_TileMetrics WHERE RunID LIKE '$RunID' AND metricCode IN " .
		"(SELECT MAX(metricCode) FROM specimen.MiSeqQC_TileMetrics WHERE RunID LIKE '$RunID' AND metricCode LIKE '20%' AND MOD(metricCode,2) = 0)";
$sth = $db->prepare($query); $sth->execute();
...
$query =        "SELECT ROUND(AVG(value)*100,3) FROM specimen.MiSeqQC_TileMetrics WHERE RunID LIKE '$RunID' AND metricCode IN " .
                "(SELECT MIN(metricCode) FROM specimen.MiSeqQC_TileMetrics WHERE RunID LIKE '$RunID' AND metricCode LIKE '20%' AND MOD(metricCode,2) = 1)";
$sth = $db->prepare($query); $sth->execute();
...
$query =        "SELECT ROUND(AVG(value)*100,3) FROM specimen.MiSeqQC_TileMetrics WHERE RunID LIKE '$RunID' AND metricCode IN " .
                "(SELECT MAX(metricCode) FROM specimen.MiSeqQC_TileMetrics WHERE RunID LIKE '$RunID' AND metricCode LIKE '20%' AND MOD(metricCode,2) = 1)";
$sth = $db->prepare($query); $sth->execute();
...
$query = "SELECT 1-(SUM(numPerfectReads) / SUM(numPerfectReads+numreadsoneerror+numreadstwoerror+numreadsthreeerror+numreadsfourerror)) errorRate, cycle " .
	 "FROM specimen.MiSeqQC_ErrorMetrics WHERE runID LIKE '$RunID' AND cycle = $read_1_cycle_35 GROUP BY cycle ORDER BY cycle";
$sth = $db->prepare($query); $sth->execute();
...
$query = "SELECT 1-(SUM(numPerfectReads) / SUM(numPerfectReads+numreadsoneerror+numreadstwoerror+numreadsthreeerror+numreadsfourerror)) errorRate, cycle " .
         "FROM specimen.MiSeqQC_ErrorMetrics WHERE runID LIKE '$RunID' AND cycle = $read_2_cycle_35 GROUP BY cycle ORDER BY cycle";
$sth = $db->prepare($query); $sth->execute();
...
$query = 	"INSERT INTO specimen.MiSeqQC_InterOpSummary " .
